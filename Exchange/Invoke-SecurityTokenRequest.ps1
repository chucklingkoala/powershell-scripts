# Description: Requests a SAML security token from an ADFS server using Windows or Username authentication
# Requirements: .NET System.ServiceModel and System.IdentityModel assemblies
function Invoke-ADFSSecurityTokenRequest {
    ...
    switch ($ClientCredentialType) {
        'Windows'  { $MessageCredential = 'Windows';   $ADFSTrustEndpoint = 'windowsmixed' }
        'UserName' { $MessageCredential = 'UserName';  $ADFSTrustEndpoint = 'usernamemixed' }
    }
    ...
    $EP = New-Object -TypeName System.ServiceModel.EndpointAddress -ArgumentList (
        '{0}/{1}/{2}' -f $ADFSBaseUri, $ADFSTrustPath, $ADFSTrustEndpoint
    )
    ...
    if ($OutputType -eq 'RSTR') { Write-Output $RSTR } else { Write-Output $Token }
}
